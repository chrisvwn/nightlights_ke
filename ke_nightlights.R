...
yearly_means <- aggregate(nl_means, by=list(year(nl_dates)), mean)
...
aggregate(colMeans(extract[,nl_cols]), by=list(extract[,"constituency_name"]), mean)

#melt the yearmonth columns into a single column to facilitate panel plotting
extractMelted <- melt(data = extract, id.vars = "constituency_name", measure.vars = nl_cols, variable.name = "NL")

extractMelted$date <- as.Date(paste0(str_replace(extractMelted$NL,"NL_",""),"01"), format="%Y%m%d")

xyplot(value ~ date | county_name, data = extractMelted, type="l")

xyplot(value ~ date | county_name, data = extractMelted, type=c("l","smooth"), scales=list(y=list(log=T, equispaced.log = FALSE)))

ggplot(data = extractMelted, aes(x=date, y=value))+geom_line()+scale_y_log10()+facet_wrap(facets = ~ county_name, ncol = 4) + geom_smooth(method = "lm", na.rm =T) 
